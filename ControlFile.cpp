...
#define CF_READ_LOCK_HEADER FileSystem::AdvisoryAutoUnlock lock(mFD, sizeof(header_t), sizeof(Header), false)
#define CF_WRITE_LOCK_HEADER FileSystem::AdvisoryAutoUnlock lock(mFD, sizeof(header_t), sizeof(Header), true)


// statics
template < typename Header, typename Record >
const unsigned ControlFile<Header, Record>::sLockTimeout = 60;
...
{
    struct stat st;
    bool ret = (FileSystem::get()->stat(mFilename, &st) == 0);
...
void ControlFile<Header, Record>::createEmpty()
{
    ...
    // delete any old one
    FileSystem::get()->unlink(mFilename);

    // open control file for read-write
    if ((mFD = ::open(mFilename, O_RDWR|O_CREAT, 0600)) == -1)
        throw ForteControlFileException(FStringFC(), "Failed to create control file '%s': %s",
                                         mFilename.c_str(), FileSystem::get()->strerror(errno).c_str());
    ...
    if (write(mFD, &header, sizeof(header)) != sizeof(header))
...
void ControlFile<Header, Record>::setHeader(const Header &userHeader)
{
    ...
    {
        FileSystem::AdvisoryAutoUnlock lock(mFD, sizeof(header_t), sizeof(Header), true);
        lseek64(mFD, sizeof(header_t), SEEK_SET); // seek past our header
        if (write(mFD, &userHeader, sizeof(Header)) != sizeof(Header))
...
void ControlFile<Header, Record>::getHeader(Header &userHeader)
{
    checkOpen();
    {
        FileSystem::AdvisoryAutoUnlock lock(mFD, sizeof(header_t), sizeof(Header), false);
        lseek64(mFD, sizeof(header_t), SEEK_SET); // seek past our header
...
uint64_t ControlFile<Header, Record>::enqueue(const Record &r)
{
    ...
    // update the header
    {
        ...
        if (write(mFD, &record, sizeof(record_t)) != sizeof(record_t))
        {
            ftruncate(mFD, orig_eof); // undo everything
...
void ControlFile<Header, Record>::checkOpen(void)
{
    ...
    if ((mFD = ::open(mFilename, O_RDWR|O_NONBLOCK, 0600)) == -1)
...

    {
        ...
        if (wrapper.status != CF_STATUS_QUEUED)
            throw ForteControlFileException(FStringFC(), "control file '%s' in invalid state. "
                                             ...
                                             mFilename.c_str(), wrapper.status);
...
//    inet_aton(NetUtil::get()->getMyPrivateIP(), &addr); TODO
        ...
        if (write(mFD, &wrapper, sizeof(wrapper)) != sizeof(wrapper))
        {
        ...
        if (header.n_remaining > 0)
        {
            ...
            size_t bytesRead;
            while ((bytesRead = ::read(mFD, &wrapper, sizeof(wrapper))) == sizeof(wrapper))
            {
...
{
    // locking must be performed by the caller
    lseek64(mFD, 0, SEEK_SET); // seek to 0, rewrite header
    if (write(mFD, &header, sizeof(header_t)) != sizeof(header_t))
